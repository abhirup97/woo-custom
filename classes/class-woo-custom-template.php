...
/**
 * @class 		WooCustom Template Class
 *
 * @author      abhirup
 * @package     woo-custom/classes
 * @version     1.0.0
 */
class WooCustom_Template {

    public $template_url;

    public function __construct() {
        $this->template_url = 'woo-custom/';
    }
    ...
    public function locate_template( $template_name, $template_path = '', $default_path = '' ) {
        ...
        if ( !$default_path ) {
            $default_path = $WooCustom->plugin_path . 'templates/';
        }
        // Look within passed path within the theme - this is priority
        $template = locate_template( array( trailingslashit( $template_path ) . $template_name, $template_name ) );
        // Add support of third perty plugin
        $template = apply_filters( 'woo_custom_locate_template', $template, $template_name, $template_path, $default_path );
    ...
    public function store_locate_template( $template_name, $template_path = '', $default_path = '' ) {
        ...
        $template = locate_template( array( trailingslashit( $template_path ) . $template_name, $template_name ) );
        // Add support of third perty plugin
        $template = apply_filters( 'woo_custom_store_locate_template', $template, $template_name, $template_path, $default_path );
        // Get default template
        if ( ! $template ) {
            $template = $default_path . $template_name;
        }
    ...
    public function get_template_part( $slug, $name = '' ) {
        ...
        if ( !$template && $name && file_exists( $WooCustom->plugin_path . "templates/{$slug}-{$name}.php" ) )
            $template = $WooCustom->plugin_path . "templates/{$slug}-{$name}.php";
