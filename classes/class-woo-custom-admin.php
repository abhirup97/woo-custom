...
class WooCustom_Admin {
	public function __construct() {
		add_filter( 'woocommerce_general_settings' , [ $this, 'general_settings_shop_phone' ] );
	...
	public function general_settings_shop_phone($settings) {
		$key = 0;
		...
		foreach( $settings as $values ){
			...
			if($values['id'] == 'woocommerce_price_num_decimals'){
				$new_settings[$key] = array(
					'title'    => __('Custom fee per order'),
					'desc'     => __('Fee to add every order'),
